...
contract ClaimVerifier {
    ...
    returns (bool claimValid)
  {
    ...
    function signedAndHashed(ClaimHolder _identity, uint256 claimType, bytes data, bytes sig)
    public
    constant
    returns (address) {
        bytes32 dataHash = keccak256(_identity, claimType, data);
        bytes32 prefixHashed = keccak256("\x19Ethereum Signed Message:\n32", dataHash);
        return getRecoveredAddress(sig, prefixHashed);
    }
